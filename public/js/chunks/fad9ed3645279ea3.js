...
function loadPlayer(videoId) {
  ...
  function onStateChange(evnt) {
    ...
    console.log("player", player.getCurrentTime()); // if playing then call api after 60 seconds
    ...
    if (evnt.data == 1) {
      if (player.getCurrentTime() < 60) {
        myTimeout = setTimeout(function () {
          ...
          window.videocompo.testMethod(); // window.videocompo.youtubeRoyaltiesCount()
    ...
    } else if (evnt.data == 2) {// buffering & check playinng seconds & check timer second & call api
    }
  }
}
